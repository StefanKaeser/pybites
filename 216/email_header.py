...
def get_email_details(header: str) -> dict:
    ...
    """
    from_regex = re.compile(r"From: (?P<from>.*?)\n")
    to_regex = re.compile(r"To: (?P<to>.*?)\n")
    subject_regex = re.compile(r"Subject: (?P<subject>.*?)\n")
    date_regex = re.compile(r"Date: (?P<date>.*?) [+-].*\n")

    regexs = [from_regex, to_regex, subject_regex, date_regex]
    field_to_content = {}
    for regex in regexs:
        match = regex.search(header)
        if not match:
            return None
        field_to_content = {**field_to_content, **match.groupdict()}
    return field_to_content
