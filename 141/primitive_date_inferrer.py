...
def get_dates(dates):
    ...
    """
    possible_formats = Counter()
    for date_str in dates:
        possible_formats.update(_maybe_DateFormats(date_str))

    # Tie
    if len(set(sorted(possible_formats.values())[-2:])) == 1:
        raise InfDateFmtError("No leading format could be determined.")

    format_id = possible_formats.most_common(1)[0][0].value

    # Too many not parseable
    if format_id == -999:
        raise InfDateFmtError("Too many date strings are not parseable.")

    # Parse with most common parser
    d_parse_fmt = DateFormat.get_d_parse_formats(format_id)

    dates_return = []
    for date_str in dates:
        try:
            dates_return.append(
                datetime.strptime(date_str, d_parse_fmt).strftime("%Y-%m-%d")
            )
        except ValueError:
            dates_return.append("Invalid")
