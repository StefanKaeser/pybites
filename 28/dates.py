...
def get_month_most_posts(dates):
    """Receives a list of datetimes and returns the month (format YYYY-MM)
       that occurs most"""
    year_months = [date.strftime("%Y-%m") for date in dates]
    year_months_counts = collections.Counter(year_months)
    month_most_posted, _ = year_months_counts.most_common(1)[0]
    return month_most_posted
