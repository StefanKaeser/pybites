...
def _alive_at_date(composer, date_str):
    dt = _get_date(date_str)
    born_dt = _get_date(composer.born)
    died_dt = _get_date(composer.died)

    composer_alive = born_dt <= dt <= died_dt
    return composer_alive
...
def operas_both_at_premiere(guest, composer):
    ...
    operas_of_author = [opera for opera in operas if opera.author == composer_str]

    for opera in operas_of_author:
        date = opera.date

        guest_alive = _alive_at_date(guest, date)
        composer_alive = _alive_at_date(composer, date)

        if guest_alive and composer_alive:
            yield opera.play
